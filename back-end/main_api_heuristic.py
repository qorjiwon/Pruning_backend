...
def get_chatgpt_comment_with_image(base64_image_str: Optional[str], num_pruning_points: int, image_filename: str) -> str:
    ...
    try:
        prompt_text = (
            ...
            "이미지를 보고, 아래 형식에 맞게 답해라. '1.식물 이름 (단답) , 2.현재 계절 (단답), 3.언제쯤 가지치기가 적절한지 (문장 형태), 4.해당 식물의 가지치기 원칙 ( 소괄호를 이용한 다른 단락 넘버링), 5.이미지 기반 분석, 6.전정 순서 제안 ( 소괄호를 이용한 다른 단락 넘버링), 7. 수확량 증가가 몇 퍼센트 될 지 예상치'"
        )
        messages_payload = [
